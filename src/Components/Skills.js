...
export default function Skills() {
    ...
    return (
        ...
        <section id="skills" className="container mx-auto flex px-10 py-60 flex-col" >
            ...
            {skills.map(skill => (
                <div key={skill.id} className=" w-full  p-4 flex sm:flex-row md:flex:col xs:flex-col flex-col title-font items-center sm:text-xl text-3xl mb-4 font-medium text-gray-400">
                    <div className="md:w-1/4  p-4 mr-4  sm:text-2xl text-2xl mb-4">
                        <p>{skill.title}</p>
                    </div>
